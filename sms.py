...
def toplevel_data(title,button_text,command):
    ...
    genderLabel = Label(screen, text='Certificate', font=('times new roman', 20, 'bold'))
...
def connect_database():
    def connect():
        global mycursor,con
        try:
            con=pymysql.connect(host='127.0.0.1',user='root',password='vcet')
            mycursor=con.cursor()
        except:
        ...
        try:
            ...
            query='create table student(id int not null primary key, name varchar(30),mobile varchar(10),email varchar(30),' \
                  'address varchar(100),certificate varchar(20),dob varchar(20),date varchar(50), time varchar(50))'
            mycursor.execute(query)
        except:
            query='use studentmanagementsystem'
            mycursor.execute(query)
...
root.resizable(0,0)
root.title('Student Certificate Management System')

datetimeLabel=Label(root,font=('times new roman',18,'bold'))
datetimeLabel.place(x=5,y=5)
clock()
s='Student Certificate Management ' #s[count]=t when count is 1
...
studentTable=ttk.Treeview(rightFrame,columns=('Id','Name','Mobile','Email','Address','Certificate',
...
studentTable.heading('Certificate',text='Certificate')
...
studentTable.column('Certificate',width=100,anchor=CENTER)
